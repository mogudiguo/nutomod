...
public class EnergyBootsItem extends ArmorItem {
    
    public EnergyBootsItem() {
    ...
    // 当玩家穿着靴子时，每 tick 检测一次附近是否有激活的能量核心
    ...
    public void appendTooltip(ItemStack stack, List<Text> tooltip) {
        ...
        tooltip.add(Text.literal("§5§l👢 能量靴子").formatted(Formatting.DARK_PURPLE, Formatting.BOLD));
        ...
        tooltip.add(Text.literal("   - 耐久度：1456（钻石的 3.5 倍）").formatted(Formatting.GREEN));
        tooltip.add(Text.literal("   - 防火、防爆炸").formatted(Formatting.GREEN));
        tooltip.add(Text.literal("§b● 激活时效果:").formatted(Formatting.AQUA));
        tooltip.add(Text.literal("   - 🦘 跳跃提升 I（跳得更高）").formatted(Formatting.GOLD));
        tooltip.add(Text.literal("   - 💨 速度提升 I（移动加速）").formatted(Formatting.GRAY));
